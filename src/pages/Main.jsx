...
const Main = () => {
  ...
  const searchMovie = async (e) => {
    ...
    console.log(urlNew);
      ...
      .then(handleErrors)
      .then((response) => response.json())
      .then((data) => {window.sessionStorage.setItem("data", JSON.stringify(data.results));setData(data.results);setTotalPage(data.total_pages);})
      .catch((error) => console.log(error));
      ...
      {user?.email ? (
        ...
        >
          <TextField
            ...
            onChange={(e) => {
              setSearch({ ...search, search1: e.target.value });
            }}
          />
          <Button
          ...
          {search?.search2  ? (
            <div style={{ textAlign: "center",marginTop:"5px" }}>
              {page>1 && <Button
                sx={{ textAlign: "center", height: "56px",marginRight:"5px" }}
                type="submit"
                variant="contained"
